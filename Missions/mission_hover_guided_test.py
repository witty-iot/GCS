...
Flow:
  arm -> takeoff to HOVER_ALTITUDE -> hold position in GUIDED for
  HOVER_SECONDS (straight up, hover in place, then land) -> land

The hold step is a "local_ned" step with a zero north/east/down offset:
the ESP32 computes an absolute target at the vehicle's position the
moment takeoff completes, then actively resends that same absolute
target every 2s for the duration of the hold. This is a firmer hold than
a plain "wait" (which sends nothing at all and just hopes the vehicle
stays on whatever target takeoff left it on) — it gives the ESP32 an
explicit position to supervise (distance/altitude error is checked every
loop) and a timeout to fall back on if the vehicle can't hold position.
...
MISSION = {
    "steps": [
        ...
        {"action": "local_ned", "x": 0, "y": 0, "z": 0, "seconds": HOVER_SECONDS},
        {"action": "land"},
    ]
}
...
def send_start():
    ...
    print(f"      Expect: arm -> takeoff to {HOVER_ALTITUDE}m -> hold position {HOVER_SECONDS}s in GUIDED -> land\n")
    if logger:
...
def main():
    ...
    logger = FlightLogger(args.mav_port, run_name="hover_guided", mission=MISSION)
    logger.start()
